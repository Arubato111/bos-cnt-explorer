...
type RespOK = {
  ...
  totalHolders: number;
  circulating: number;
  top: { address: string; raw: number; scaled: number }[];
};
type Resp = RespOK | { ok: false };

function fmt(n: number | null | undefined, f = 2) {
...
export default function HoldersLive() {
  ...
  const ok = Boolean(data && "ok" in data && (data as any).ok);
  const v = ok ? (data as RespOK) : undefined;
  ...
  return (
    <section className="rounded-2xl bg-white/5 border border-white/10">
      ...
      <div className="p-4 grid md:grid-cols-3 gap-4 text-sm">
        ...
        <Card label="Decimals" value={ok ? String(v?.decimals ?? "Not available yet") : "Not available yet"} />
      </div>
